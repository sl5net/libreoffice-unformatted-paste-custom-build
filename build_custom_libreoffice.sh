...
PATCH_FILES=(
    "libreoffice-accelerators-ctrl-v-unformatted.patch"
    "libreoffice-calc-pasteunformatted-skipdialog.patch"
)
...
apply_custom_patches() { # Renamed function
    local script_dir
    script_dir=$(dirname "$(readlink -f "$0")")

    cd "$LO_SOURCE_DIR"
    log_info "Attempting to revert any old patches first (best effort)..."
    for patch_name in "${PATCH_FILES[@]}"; do
        local actual_patch_file="${script_dir}/${patch_name}"
        if [ -f "$actual_patch_file" ]; then
            patch -p1 -R --dry-run < "$actual_patch_file" && patch -p1 -R < "$actual_patch_file" || true
        fi
    done
    git reset --hard HEAD # Ensure clean state after potential reverse patches

    log_info "Applying new patches..."
    for patch_name in "${PATCH_FILES[@]}"; do
        local actual_patch_file="${script_dir}/${patch_name}"
        if [ ! -f "$actual_patch_file" ]; then
            log_error "Patch file '$actual_patch_file' not found."
            exit 1
        fi
        ...
        if patch -p1 --dry-run < "$actual_patch_file"; then
            ...
            log_error "Patch application for '$actual_patch_file' would fail. Please check the patch."
            exit 1
        fi
    done
    cd - > /dev/null
